import { TrendingDown, XCircle, AlertTriangle, Clock, Shield, Target, Users, BarChart3, PieChart, Activity, Zap, TrendingUp, Award, Globe, Heart, Star, CheckCircle, X, Check, Lightbulb, Rocket, ArrowRight, DollarSign, ArrowUpRight, ArrowDownRight } from 'lucide-react';
...
export default function WeaknessesSlide({ slide, theme: themeProp, isThumbnail }: WeaknessesSlideProps) {
  const t = themeProp;
  const { content } = slide;
    ...
    >
      ...
      <div className="flex-1 px-8 pb-6 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 h-full">
          {(highlights.length > 0 ? highlights : items.map((it) => ({ icon: 'AlertTriangle', title: it.split(/[:\-–]/)[0]?.trim() || it, description: it.includes(':') || it.includes(' - ') || it.includes(' – ') ? it.split(/[:\-–]/).slice(1).join(':').trim() : '' }))).map((hl, index) => {
            const ItemIcon = getIcon(hl.icon);
            const accentColor = t.colors.chart4;
            const amberColor = t.colors.chart5;

            return (
              <div
                key={index}
                className={cn(
                  'rounded-xl border flex overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-lg',
                  isThumbnail && 'p-2'
                )}
                style={{
              ...
              >
                {/* Left accent - alternating red/amber */}
                <div
                  className="w-1.5 shrink-0"
                  style={{ background: index % 2 === 0 ? accentColor : amberColor }}
                />
